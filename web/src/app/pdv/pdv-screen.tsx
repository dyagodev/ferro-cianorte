...
import type { Cliente, ItemVendavel, Loja, NotaFiscal, Produto, Servico } from "@/lib/types";
...
export default function PdvScreen({
  ...
  const [notasFiscaisEmitidas, setNotasFiscaisEmitidas] = useState<NotaFiscal[]>([]);
  ...
  async function confirmarVenda(
    ...
    const venda = await apiFetch<{ id: number; created_at: string; notas_fiscais?: NotaFiscal[] }>("vendas", {
      method: "POST",
      body: JSON.stringify({
    ...
    setNotasFiscaisEmitidas(venda.notas_fiscais ?? []);
  ...
  async function imprimirCupom() {
    await imprimir();
    fecharVendaConcluida();
  }

  function fecharVendaConcluida() {
    setUltimaVenda(null);
    setNotasFiscaisEmitidas([]);
  }
      ...
      {ultimaVenda && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/60">
          <div className="w-full max-w-sm rounded-lg border border-slate-300 bg-slate-50 p-6 text-center shadow-2xl">
            ...
            <p className="mb-4 text-slate-600">Total: R$ {ultimaVenda.total.toFixed(2)}</p>

            {notasFiscaisEmitidas.length > 0 && (
              <div className="mb-4 rounded border border-slate-200 bg-white p-3 text-left text-sm">
                {notasFiscaisEmitidas.map((nota) => (
                  <div key={nota.id} className="flex items-center justify-between gap-2 py-1">
